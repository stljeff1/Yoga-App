...
app.controller('PoseListCtrl', ['$scope', '$location', 'appService', function ($scope, $location, appService ) {
	...
	appService.loadTags('poseTypes').then(function(response) {
		angular.forEach(response.data, function(data) {
			...
			console.log('tag push', $scope.tags);
		})
	}, function(response) {
				...
				if(newVal) {
					...
					angular.forEach($rows, function(row) {
						...
						$row.on('mouseenter', function() {
							...
							console.log('hover', $scope.activePoseImg);
